...
2. Fetch official docs or search the web using the available tools.
...
Keep durable workarounds in visible, maintained instruction files; do not silently save them to agent memory.
...
- Repo-specific → that repo's instruction file, or its `docs/` when it needs room.
- Machine- or setup-specific → the machine-local instruction location described below.

Record what would make the workaround unnecessary, so it can be deleted once that lands instead of outliving the problem.

## Code style

Follow existing style in the codebase, or rules stated in project-specific instruction files.
...
Follow language-specific rules when their stated file scope matches the work.
...
Two questions decide whether a rule or a fact earns prose in an instruction-file line, a doc paragraph, or a comment. Both answers must be yes.
